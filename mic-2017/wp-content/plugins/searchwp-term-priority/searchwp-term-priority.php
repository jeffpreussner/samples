...
if ( ! defined( 'SEARCHWP_TERM_PRIORITY_VERSION' ) ) {
	define( 'SEARCHWP_TERM_PRIORITY_VERSION', '1.1.3' );
}
							...
							foreach ( $post_type['weights']['tax'] as $taxonomy => $weight ) {
								...
								if ( intval( $weight ) > 0 ) {
									...
									if (
										...
										$args = apply_filters( 'searchwp_term_archive_term_args', $args );
										...
										elseif ( $term_id = term_exists( implode( ' ', $terms ), $taxonomy ) ) {
											...
											$defaults = apply_filters( 'searchwp_term_archive_term_args', $defaults );
